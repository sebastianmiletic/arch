...
function buildRequest(provider: ProviderConfig, messages: Message[]) {
  ...
  switch (provider.id) {
    case 'ollama': {
      if (!provider.baseUrl) throw new Error('No base URL for Ollama');
    ...
    case 'anthropic': {
      if (!provider.baseUrl) throw new Error('No base URL for Anthropic');
    ...
    case 'gemini': {
      if (!provider.baseUrl) throw new Error('No base URL for Gemini');
    ...
    case 'openrouter': {
      if (!provider.baseUrl) throw new Error('No base URL for OpenRouter');
    ...
    case 'nvidia': {
      if (!provider.baseUrl) throw new Error('No base URL for NVIDIA');
      url = `${provider.baseUrl}/chat/completions`;
      if (provider.apiKey) headers['Authorization'] = `Bearer ${provider.apiKey}`;
      body = JSON.stringify({
        model: provider.defaultModel,
...
function parseResponse(providerId: string, data: any, fallbackModel: string): { content: string; model: string; tokens?: number } {
  switch (providerId) {
    case 'ollama': {
...
async function chatOpencode(messages: Message[]): Promise<{ content: string; model: string; tokens?: number }> {
  try {
    ...
    const result = await new Promise<string>((resolve, reject) => {
      ...
      child.stdin.end();
      child.on('close', (code) => {
        if (code !== 0) reject(new Error(`OpenCode exited ${code}: ${stderr || ''}`));
        else resolve(stdout);
      });
    ...
    for (const line of lines) {
      try {
        ...
        if (obj.type === 'text' && obj.part?.text) content += obj.part.text;
...
export async function testProvider(config: ProviderConfig): Promise<{ ok: boolean; latency: number; models?: string[]; error?: string }> {
  ...
  try {
    ...
    const tempConfig: ProviderConfig = { ...config, defaultModel: useModel };
