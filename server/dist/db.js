...
export function getCodeChanges(limit = 100) {
...
export function addCodeChange(change) {
    db.prepare(`
    INSERT INTO code_changes (id, file_path, change_type, line_start, line_end, old_content, new_content, timestamp, reason, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(change.id, change.filePath, change.changeType, change.lineStart, change.lineEnd, change.oldContent || null, change.newContent || null, change.timestamp, change.reason, change.status);
}
export function updateCodeChangeStatus(id, status) {
    db.prepare('UPDATE code_changes SET status = ? WHERE id = ?').run(status, id);
}
export function getLatestLoopState() {
    const row = db.prepare('SELECT * FROM loop_states ORDER BY start_time DESC LIMIT 1').get();
    if (!row)
        return undefined;
    return {
        id: row.id,
...
export function createLoopState(state) {
...
export function updateLoopState(id, updates) {
    const sets = [];
    const values = [];
    ...
    values.push(id);
    if (sets.length)
        db.prepare(`UPDATE loop_states SET ${sets.join(', ')} WHERE id = ?`).run(...values);
}
export function getLoopLogs(stateId) {
    const rows = db.prepare('SELECT * FROM loop_logs WHERE state_id = ? ORDER BY timestamp ASC').all(stateId);
    return rows.map(r => ({
        timestamp: r.timestamp,
        stage: r.stage,
        level: r.level,
...
export function addLoopLog(stateId, log) {
