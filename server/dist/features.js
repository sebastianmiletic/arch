...
export function seedFeatures() {
    const row = db.prepare('SELECT count(*) as c FROM features').get();
    if (row.c > 0)
        return;
...
export function getFeatures() {
    const rows = db.prepare('SELECT * FROM features ORDER BY priority ASC').all();
    return rows.map(r => ({
        id: r.id,
        name: r.name,
...
export function updateFeature(id, updates) {
    ...
    values.push(id);
    if (sets.length)
        db.prepare(`UPDATE features SET ${sets.join(', ')} WHERE id = ?`).run(...values);
}
